...
bool Maze::find_start (size_t &row, size_t & column)
{
  ...
  for(size_t i = zero; i < maze.size(); i ++)
  {
    for(size_t n = zero; n < maze[i].length(); n++)
    {
      if(maze[i][n] == START)
      {
        row = i;
...
bool Maze::find_path (size_t row, size_t column)
{
  ...
  for(auto line: this -> maze)
  {
    number_of_columns = line.length();
    number_of_rows++;
  }
  if(row > number_of_rows - one || column > number_of_columns - one)
  {
    return false;
  }
  ...
  if(maze[row][column] == WALL || maze[row][column] == EXPLORED|| maze[row][column] == PATH)
  {
      return false;
  }
